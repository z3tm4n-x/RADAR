...
def _spectrum(
    *,
    ...
    return Spectrum1D(
        ...
        particle=particle,
        source=source,
        model="sep_proton_penetrated",
    )


def test_sep_proton_shielding_zero_thickness_preserves_total_spectrum() -> None:
...
def test_sep_proton_shielding_total_includes_secondary_when_enabled() -> None:
    result_without_secondary = calculate_sep_proton_shielding_spectrum(
        spectrum=_spectrum(),
        tables=_tables(survival_sigma_barn=0.0, secondary_sigma_barn=1000.0),
        thickness_g_cm2=1.0,
        include_secondary_protons=False,
    )
    result_with_secondary = calculate_sep_proton_shielding_spectrum(
        spectrum=_spectrum(),
        tables=_tables(survival_sigma_barn=0.0, secondary_sigma_barn=1000.0),
        thickness_g_cm2=1.0,
...
def test_sep_proton_shielding_can_disable_nonelastic_survival() -> None:
    without_survival = calculate_sep_proton_shielding_spectrum(
        spectrum=_spectrum(),
        tables=_tables(survival_sigma_barn=1000.0),
        thickness_g_cm2=1.0,
        include_nonelastic_survival=False,
    )
    with_survival = calculate_sep_proton_shielding_spectrum(
        spectrum=_spectrum(),
        tables=_tables(survival_sigma_barn=1000.0),
        thickness_g_cm2=1.0,
...
def test_sep_proton_shielding_product_preserves_product_kind_and_units() -> None:
    ...
    shielded = calculate_sep_proton_shielding_product(
        product=product,
        tables=_tables(),
        thickness_g_cm2=0.0,
    )
...
def test_sep_proton_shielding_product_supports_peak_flux_product() -> None:
    product = SpectrumProduct(
...
def test_sep_proton_shielding_model_records_method_flags() -> None:
    result = calculate_sep_proton_shielding_spectrum(
        spectrum=_spectrum(),
        tables=_tables(),
        thickness_g_cm2=0.0,
    ...
    assert SEP_PROTON_AL_SHIELDING_MODEL in result.total.model
    assert SEP_PROTON_AL_CENTERED_SPHERICAL_SHELL in result.total.model
    assert SEP_PROTON_AL_NO_HIDDEN_4PI in result.total.model
    assert "proton_nonelastic_survival_included" in result.total.model
    assert "secondary_protons_included" in result.total.model


def test_sep_proton_shielding_rejects_non_proton_spectrum() -> None:
...
def test_sep_proton_shielding_rejects_non_sep_spectrum() -> None:
...
def test_sep_proton_shielding_rejects_negative_thickness() -> None:
