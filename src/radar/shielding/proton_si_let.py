...
PROTON_SI_LET_MODEL = "proton_si_let_histogram"
...
class ProtonSiLetTable:
    """Proton LET table in silicon."""
    ...
    def __post_init__(self) -> None:
        n = len(self.energy_mev)

        if n < 2:
            msg = "Proton Si LET table must contain at least two points."
        ...
        for values, name in (
            ...
            if len(values) != n:
                msg = f"Proton Si {name} array must match energy array length."
                raise ValueError(msg)
    ...
    def total_let_at_energy(self, energy_mev: float) -> float:
        """Return total LET in MeV cm²/mg at proton kinetic energy."""

        return _interpolate_strict(
            x=energy_mev,
            xp=self.energy_mev,
            fp=self.let_total_mev_cm2_mg,
            quantity="proton Si total LET",
        )


def _validate_finite(values: tuple[float, ...], name: str) -> None:
    if any(not isfinite(value) for value in values):
        msg = f"Proton Si LET table {name} values must be finite."
        raise ValueError(msg)
...
def _validate_positive(values: tuple[float, ...], name: str) -> None:
    ...
    if any(value <= 0.0 for value in values):
        msg = f"Proton Si LET table {name} values must be positive."
        raise ValueError(msg)
...
def _validate_nonnegative(values: tuple[float, ...], name: str) -> None:
    ...
    if any(value < 0.0 for value in values):
        msg = f"Proton Si LET table {name} values must be non-negative."
        raise ValueError(msg)
...
def _validate_positive_increasing(values: tuple[float, ...], name: str) -> None:
    ...
    if any(right <= left for left, right in zip(values, values[1:])):
        msg = f"Proton Si LET table {name} values must be strictly increasing."
        raise ValueError(msg)
...
def _output_quantity_and_unit(quantity: SpectrumQuantity) -> tuple[SpectrumQuantity, Unit]:
    ...
    msg = "Proton LET conversion requires an energy fluence or flux spectrum."
    raise ValueError(msg)


def proton_energy_spectrum_to_let_spectrum(
    *,
    spectrum: Spectrum1D,
    ...
    if spectrum.particle is not Particle.PROTON:
        msg = "Proton LET conversion requires a proton spectrum."
        raise ValueError(msg)

    if spectrum.x_unit is not Unit.MEV:
        msg = "Proton LET conversion requires an energy grid in MeV."
        raise ValueError(msg)
    ...
    return Spectrum1D(
        ...
        model=model or f"{spectrum.model}+{PROTON_SI_LET_MODEL}",
    )
...
def _required_text(row: Mapping[str, str | None], column: str, line_number: int) -> str:
    ...
    if value is None or not value.strip():
        msg = f"Missing required column {column!r} at proton Si LET CSV line {line_number}."
        raise ValueError(msg)

    return value.strip()


def load_proton_si_let_table(path: str | Path) -> ProtonSiLetTable:
    """Load normalized proton silicon LET CSV table."""
    ...
    with Path(path).open("r", encoding="utf-8", newline="") as file:
        ...
        if missing_columns:
            ...
            msg = f"Proton Si LET CSV is missing columns: {missing}."
            raise ValueError(msg)
